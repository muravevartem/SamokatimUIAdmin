...
export class CompanyService {
    ...
    async getAll(pageable: Pageable): Promise<Page<Organization>> {
        let response =
            await $apiHandbook.get<Page<Organization>>(`${CompanyService.BASE_URL}?page=${pageable.page}&size=${pageable.size}`);
        return response.data;
    }

    async searchPaged(keyword: string, pageable: Pageable): Promise<Page<Organization>> {
        let response =
            await $apiHandbook.get<Page<Organization>>(`${CompanyService.BASE_URL}?search=${keyword}&page=${pageable.page}&size=${pageable.size}`);
        return response.data;
    }

    async searchByInn(inn: string): Promise<Organization> {
        let response =
            await $apiHandbook.get<Organization>(`${CompanyService.FNS_URL}?inn=${inn}`);
        return response.data;
    }

    async getOne(orgId: number): Promise<Organization> {
        let response =
            await $apiHandbook.get<Organization>(`${CompanyService.BASE_URL}/${orgId}`);
        return response.data;
    }

    async create(org: Organization): Promise<Organization> {
        let response =
            await $apiHandbook.post<Organization>(`${CompanyService.BASE_URL}`, org);
        return response.data;
    }

    async modify(orgId: number, org: Organization): Promise<Organization> {
        let response =
            await $apiHandbook.put<Organization>(`${CompanyService.BASE_URL}/${orgId}`, org);
        return response.data;
    }

    async approve(orgId: number): Promise<Organization> {
        let response =
            await $apiHandbook.put<Organization>(`${CompanyService.BASE_URL}/${orgId}/approve`);
        return response.data;
    }

    async delete(orgId: number): Promise<null> {
        return await $apiHandbook.delete(`${CompanyService.BASE_URL}/${orgId}`)
    }
}
